...
def main(args):
    ...
    pipe = StyleGallery.from_pretrained(
        ...
        scheduler=scheduler, 
        safety_checker=None,
        torch_dtype=torch.float16  # 加上 fp16 可以大幅节省显存
    ).to(device)
    ...
    # 核心优化 3：切换到 LCM 调度器并融合权重
    print(f">>> Switching to LCM for Stage 2 (Optimization) using {args.lcm_lora_id}...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lcm_lora_id", type=str, default="lcm-lora-sd1.5", help="path to LCM LoRA")  # 新增 LCM 路径
    ...
    parser.add_argument("--num_optimize_steps", type=int, default=30) 
    parser.add_argument("--iters", type=int, default=2) 
    parser.add_argument("--lr", type=float, default=0.25) 
    parser.add_argument("--c_ratio", type=float, default=0.28) 
